 ...
 */
class Maketok_ProductReviews_Adminhtml_ProductreviewsController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    {
        $this->_title($this->_getHelper()->__('Maketok'))->_title($this->_getHelper()->__('Manage Customer Reviews'));

        $this->loadLayout();
        $this->_setActiveMenu('maketok');
        $this->_addContent($this->getLayout()->createBlock('productreviews/adminhtml_review'));
        $this->renderLayout();
    }

    /**
     * @return Maketok_ProductReviews_Helper_Data
     */
     ...
     *
     * @return Maketok_ProductReviews_Model_Review|bool
     */
    ...
    {
        /** @var $label Maketok_ProductReviews_Model_Review */
        $review = $this->_initReview();
        if ($review) {
            $this->_title("Review from {$review->getNickname()}");

            $this->_title($this->_getHelper()->__('Maketok'))
            ...
            $this->_getSession()->addError(
                $this->_getHelper()->__('Could not initialize the Maketok Product Review.')
            );
            $this->_redirect('*/*/grid');
        }
    }
        ...
        if ($review) {
            try {
                ...
                $this->_getSession()->addSuccess(
                    $this->_getHelper()->__('The Maketok Product Review has been successfully deleted.')
                );
            } catch (Exception $e) {
                $this->_getSession()->addError(
                    $this->_getHelper()->__('Could not delete Maketok Product Review.')
                );
                Mage::logException($e);
            }
        } else {
    ...
    public function approveAction()
    {
        ...
        if ($review) {
            try {
                $review->setStatusId(Maketok_ProductReviews_Model_Review::STATUS_APPROVED)->save();
                $this->_getSession()->addSuccess(
                    $this->_getHelper()->__('The Maketok Product Review has been successfully approved.')
                );
            } catch (Exception $e) {
                $this->_getSession()->addError(
                    $this->_getHelper()->__('Could not approve Maketok Product Review.')
                );
                Mage::logException($e);
            }
        } else {
    ...
    public function declineAction()
    {
        ...
        if ($review) {
            try {
                $review->setStatusId(Maketok_ProductReviews_Model_Review::STATUS_DECLINED)->save();
                $this->_getSession()->addSuccess(
                    $this->_getHelper()->__('The Maketok Product Review has been successfully declined.')
                );
            } catch (Exception $e) {
                $this->_getSession()->addError(
                    $this->_getHelper()->__('Could not decline Maketok Product Review.')
                );
                Mage::logException($e);
            }
        } else {
    ...
    public function saveAction()
    {
        ...
        if ($review) {
            try {
                ...
                $this->_getSession()->addSuccess(
                    $this->_getHelper()->__('The Maketok Product Review has been successfully saved.')
                );
            } catch (Exception $e) {
                $this->_getSession()->addError(
                    $this->_getHelper()->__('Could not save Maketok Product Review.')
                );
                Mage::logException($e);
            }
        } else {
